 ...
 * ---------------------------------------------------------------------
 * 
 * CÓDIGO CONTÉM MODIFICAÇÕES PERSONALIZADAS
 * PROCURE POR 'CÓDIGO MODIFICADO' PARA ENCONTRAR AS MODIFICAÇÕES REALIZADAS.
 * 
 * ---------------------------------------------------------------------
   ...
   function transferDropdownLocation($locID) {
      if ($locID > 0) {
         ...
         if ($location->getFromDB($locID)) {
            ...
            /*
               CÓDIGO MODIFICADO: AO TRANSFERIR, A LOCALIZAÇÃO SERÁ ZERADA
            */
   ...
   function transferAffectedLicense($ID) {
      ...
      if ($item_softwarelicense->getFromDB($ID)) {
         if ($license->getFromDB($item_softwarelicense->getField('softwarelicenses_id'))) {
            ...
            /*
               CÓDIGO MODIFICADO: A CONDICIONAL AGORA INCLUI O VALOR 1 E TAMBÉM O -1, ALÉM
               DA SEÇÃO 'DROP LICENSE' TER SIDO DESATIVADA
               PARA QUE AS LICENÇAS NÃO SEJAM EXCLUÍDAS CONFORME REGRAS DE NEGÓCIO NATIVAS
               DO GLPI   
            */
            //// Update current : decrement number by 1 if valid
            if ($license->getField('number') >= 1 || $license->getField('number') == -1) {

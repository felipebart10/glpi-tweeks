 ...
 * ---------------------------------------------------------------------
 * 
 * CÓDIGO EDITADO PARA CRIAÇÃO DE NOVOS CARTÕES
 * PROCURE POR "CÓDIGO MODIFICADO" PARA ENCONTRAR AS ALTERAÇÕES
 * WIDGET DE RESUMO ALTERADA PARA RETORNAR INFORMAÇÕES PERSONALIZADAS
 * 
 * ---------------------------------------------------------------------
...
class Provider extends CommonGLPI
{
    ...
    * - 'assigned_for_user': tickets assigned for connected user
    * - 'notclosed': tickets which are not closed
    * - 'processing': tickets with 'processing' status
   ...
   static function nbTicketsGeneric(
      ...
      switch ($case) {
         ...
         case 'observed_tickets':
            ...
            // CÓDIGO MODIFICADO POR FELIPE BARTOCCI: TICKETS ABERTOS
         case 'not_closed':
            $status = Ticket::CLOSED;
            $params['icon']  = "fas fa-archive";
            $params['label'] = __("open tickets");
            $search_criteria = [
               [
                  'field'      => 12,
                  'searchtype' => 'equals',
            ...
            // CÓDIGO MODIFICADO POR FELIPE BARTOCCI: TICKETS EM PROCESSAMENTO
         case 'processing':
            $params['icon']  = "fas fa-archive";
            $params['label'] = __("Processando");
            $search_criteria = [
               [
                  'field'      => 12,
                  'searchtype' => 'equals',
   ...
   public static function getTicketsEvolution(array $params = []): array
   {
      ...
      if (
         isset($params['apply_filters']['dates'])
   ...
   public static function nbTicketsActor(
      ...
      // CÓDIGO MODIFICADO POR FELIPE BARTOCCI
      // FILTRA APENAS USUÁRIOS ATIVOS E QUE SÃO TÉCNICOS
      $where = [
         "$t_table.is_deleted" => 0,
         "$ug_table.is_active" => 1,
         "$ug_table.groups_id" => [1, 2],
      ];
   ...
   public static function getTicketSummary(array $params = [])
   {
      ...
      $notclosed   = self::nbTicketsGeneric('not_closed', $params);
      $incoming   = self::nbTicketsGeneric('incoming', $params);
      $processing   = self::nbTicketsGeneric('processing', $params);
      $solved   = self::nbTicketsGeneric('solved', $params);
      $waiting    = self::nbTicketsGeneric('waiting', $params);
      $late = self::nbTicketsGeneric('late', $params);

      // CÓDIGO MODIFICADO
      return [
         'data'  => [
            [
               'number' => $notclosed['number'],
               'label'  => __("Abertos"),
               'url'    => $notclosed['url'],
               'color'  => '#266ae9',
            ], [
               ...
               'label'  => __("Processando"),
               'url'    => $processing['url'],
               'color'  => '#3bc519',
            ],
            [
               'number' => $solved['number'],
               'label'  => __("Solucionados"),
               'url'    => $solved['url'],
               'color'  => '#000000',
            ], [

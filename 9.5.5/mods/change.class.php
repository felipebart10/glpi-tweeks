   ...
   **/
   ...
   function post_addItem() {
      ...
      if (isset($this->input['_problems_id'])) {
         $change = new Problem();
         if ($change->getFromDB($this->input['_problems_id'])) {
   ...
   function showForm($ID, $options = []) {
      ...
      if (!$ID) {
         ...
         if (isset($options['problems_id']) || isset($options['_problems_id'])) {
            ...
            $change = new Problem();
            if ($change->getFromDB($problems_id)) {
               ...
               $options['entities_id']         = $change->getField('entities_id');
            }
         }
      }
            ...
            foreach ($tt->predefined as $predeffield => $predefvalue) {
               if (isset($default_values[$predeffield])) {
                  ...
                  if (((count($options['_predefined_fields']) == 0)
                      ...
                      || (isset($change) && $options[$predeffield] == $change->getField($predeffield))
                  ) {
   ...
   **/
      ...
      switch ($status) {
         ...
         case "process" : // on affiche les problemes planifi??s ou assign??s au user
            $WHERE = array_merge(
               $WHERE,
               $search_assign,
               ['status' => [self::INCOMING, self::ACCEPTED, self::EVALUATION, self::APPROVAL]]
            );
            break;
         ...
         if ($showgroupchanges) {
            switch ($status) {
               ...
               case "waiting" :
                  ...
                  echo "<a href=\"".$CFG_GLPI["root_doc"]."/front/change.php?".
               ...
               case "process" :
                  $options['criteria'][0]['field']      = 12; // status
                  $options['criteria'][0]['searchtype'] = 'equals';
                  $options['criteria'][0]['value']      = [self::INCOMING, self::ACCEPTED, self::EVALUATION, self::APPROVAL];
                  $options['criteria'][0]['link']       = 'OR';
               ...
               default :
                  ...
                  echo "<a href=\"".$CFG_GLPI["root_doc"]."/front/change.php?".
                         Toolbox::append_params($options, '&amp;')."\">".
                         Html::makeTitle(__('Your changes in progess'), $displayed_row_count, $total_row_count)."</a>";
            }
            ...
            switch ($status) {
               ...
               case "process" :
                  ...
                  $options['criteria'][1]['searchtype'] = 'equals';
                  $options['criteria'][1]['value']      = [self::INCOMING, self::ACCEPTED, self::EVALUATION, self::APPROVAL];
                  $options['criteria'][1]['link']       = 'AND';
   ...
   static function showVeryShort($ID, $forcetab = '') {
      ...
      if ($change->getFromDBwithData($ID, 0)) {
         ...
         echo "<tr class='tab_bg_2'>";
         echo "<td colspan='6' ><i>".__('No change in progress.')."</i></td></tr>";
      }
   }
}

...
/**
 * 
 * CÓDIGO MODIFCICADO PARA ORDENAR AS TAREFAS DE ACORDO COM O ID
 * 
 * ---------------------------------------------------------------------
...
class ProjectTask_Ticket extends CommonDBRelation{
   ...
   **/
   static function showForProjectTask(ProjectTask $projecttask) {
      ...
      // CÓDIGO MODIFICADO: O ITERATOR AGORA É UMA QUERY
      $iterator = $DB->request([
         'FROM'         => 'glpi_projecttasks_tickets',
         'FIELDS' =>  'tickets_id',
         'WHERE'        => [
            'glpi_projecttasks_tickets.projecttasks_id'   => $ID
         ],
      ...
      while ($data = $iterator->next()) {
         $tickets[$data['tickets_id']] = $data; // CÓDIGO MODIFICADO
         $used[$data['tickets_id']]    = $data['tickets_id']; // CÓDIGO MODIFICADO
      }
      ...
      if ($numrows) {
         ...
         foreach ($tickets as $data) {
            Session::addToNavigateListItems('Ticket', $data["tickets_id"]); // CÓDIGO MODIFICADO
            Ticket::showShort($data['tickets_id'], ['followups'              => false,
                                                 'row_num'                => $i,
